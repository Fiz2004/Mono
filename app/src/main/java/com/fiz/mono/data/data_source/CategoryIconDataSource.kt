...
class CategoryIconDataSource {
    var categoryIcons: MutableList<CategoryIconUiState> = mutableListOf(
    ...
    fun select(position: Int) {
        if (!categoryIcons[position].selected) {
            categoryIcons.find { it.selected }?.selectedFalse()
        }

        categoryIcons[position].invertSelected()
    }
    ...
    fun getIDCategoryIcon(imgSrc: Int): String {
        return categoryIcons.first { it.imgSrc == imgSrc }.id
    }
}